...
类别名称 数字标号 R G B
```
数字标号不一定是连续的，比如目前只有17类，但是最大的标号是40。在处理数据的过程中，会自动将不连续的标号转换成连续的标号，从第0行开始，这一类在第几行就会被转换成几。只要用VS Code在整个目录里搜索compress_map，就能找到所有使用了这一转换过程的地方，以后需要修改的时候只要在这些地方修改就可以了。
